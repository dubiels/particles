...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const particleColors = ['#5d9e75', '#87c9c7', '#4f9c81', '#115426', '#76b524', '#4f659c'];
    ...
    document.getElementById('fetchAndSetParticles').addEventListener('click', async function () {
        try {
            const apiKey = '54c3d78c0a5207198b48229c8b317435';
            // Get city from input field
            const cityName = encodeURIComponent(document.getElementById('cityInput').value);
            ...
            const no2Level = await fetchAirPollution(apiKey, latitude, longitude, 'no2');
            ...
            const o3Level = await fetchAirPollution(apiKey, latitude, longitude, 'o3');
            // Calculate the number of O3 particles based on the level
            const o3ParticlesNumber = Math.round(o3Level);

            // Fetch SO2 level from OpenWeatherMap API
            const so2Level = await fetchAirPollution(apiKey, latitude, longitude, 'so2');
            // Calculate the number of O3 particles based on the level
            const so2ParticlesNumber = Math.round(so2Level);

            // Fetch PM 2.5 level from OpenWeatherMap API
            const pm2_5Level = await fetchAirPollution(apiKey, latitude, longitude, 'pm2_5');
            // Calculate the number of O3 particles based on the level
            const pm2_5ParticlesNumber = Math.round(pm2_5Level);

            // Fetch SO2 level from OpenWeatherMap API
            const pm10Level = await fetchAirPollution(apiKey, latitude, longitude, 'pm10');
            // Calculate the number of O3 particles based on the level
            const pm10ParticlesNumber = Math.round(pm10Level);

            // Fetch NH3 level from OpenWeatherMap API
            const nh3Level = await fetchAirPollution(apiKey, latitude, longitude, 'nh3');
            // Calculate the number of NH3 particles based on the level
            const nh3ParticlesNumber = Math.round(nh3Level);

            const totalParticleCount = o3ParticlesNumber + so2ParticlesNumber + pm2_5ParticlesNumber + pm10ParticlesNumber + nh3ParticlesNumber;
            await updateParticleCountLabels(totalParticleCount, o3ParticlesNumber, so2ParticlesNumber, pm2_5ParticlesNumber, pm10ParticlesNumber, nh3ParticlesNumber);
            generateColorArray(o3ParticlesNumber, so2ParticlesNumber, pm2_5ParticlesNumber, pm10ParticlesNumber, nh3ParticlesNumber, totalParticleCount);
            ...
            particlesJS('particles-js', {
                particles: {
                    ...
                    color: {
                        value: generateColorArray(o3ParticlesNumber, so2ParticlesNumber, pm2_5ParticlesNumber, pm10ParticlesNumber, nh3ParticlesNumber, totalParticleCount),
                    },
                    shape: {
                        type: 'circle',
                        stroke: {
    ...
    async function fetchCoordinates(cityName, apiKey) {
        ...
        try {
            ...
            if (data.length === 0) {
                alert(`Please enter a valid city name, and do not include states, territories, or country codes in your query.`);
                return;
            } else {
    ...
    async function fetchAirPollution(apiKey, latitude, longitude, pollutant) {
        ...
        try {
            ...
            const pollutantLevel = parseFloat(data.list[0].components[pollutant]);
    ...
    async function updateParticleCountLabels(count, o3ParticlesNumber, so2ParticlesNumber, pm2_5ParticlesNumber, pm10ParticlesNumber, nh3ParticlesNumber, latitude, longitude, apiKey) {
        ...
        if (count == 0) {
            ...
            particleCountLabel.innerHTML = `<div class="pure-u-1-2 pure-u-xl-1-5"><img class="icon" style="margin-left: 0" src="icon/orange.png">O<sup>3</sup>: ${o3ParticlesNumber}</div><div class="pure-u-1-2 pure-u-xl-1-5"><img class="icon" src="icon/lightblue.png">SO<sup>2</sup>: ${so2ParticlesNumber}</div><div class="pure-u-1-2 pure-u-xl-1-5"><img class="icon" src="icon/lightgreen.png">PM2.5:&nbsp${pm2_5ParticlesNumber}</div><div class="pure-u-1-2 pure-u-xl-1-5"><img class="icon" src="icon/darkgreen.png">PM10: ${pm10ParticlesNumber}</div><div class="pure-u-1-2 pure-u-xl-1-5"><img class="icon" src="icon/darkblue.png">NH<sup>3</sup>: ${nh3ParticlesNumber}</div>`;
    ...
    function generateColorArray(o3ParticlesNumber, so2ParticlesNumber, pm2_5ParticlesNumber, pm10ParticlesNumber, nh3ParticlesNumber, totalParticleCount) {
        ...
        const pm2_5Percentage = (pm2_5ParticlesNumber / totalParticleCount) * 100;
